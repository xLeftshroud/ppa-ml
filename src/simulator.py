...
@dataclass
class Simulator:
    model: object            # anything with .predict(DataFrame) -> log_volume_in_litres
    ...
    def simulate(
        ...
        litres_base = float(np.expm1(self.model.predict(X_base)[0]))
        litres_scen = float(np.expm1(self.model.predict(X_scen)[0]))
        litres_scen = max(litres_scen, 0.0)
        lift_pct = (litres_scen - litres_base) / litres_base if litres_base > 0 else np.nan
        ...
        return {
            ...
            "baseline_litres": litres_base,
            "scenario_litres": litres_scen,
    ...
    def sanity_check(
        ...
        df["volume_nonneg"] = df["scenario_litres"] >= 0
        return df
